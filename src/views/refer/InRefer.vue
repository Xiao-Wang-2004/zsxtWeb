<template>
  <div class="in-refer-container">
    <div class="info-card">
      <p class="info-title">入库明细流水</p>
      ...
      <el-table :data="tableData" height="auto" style="width: 100%; margin-top: 0px; margin-bottom: 0px;">
        <el-table-column prop="entryid" label="入库单号" width="220">
        ...
        <el-table-column prop="date" label="入库时间" width="220" sortable />
        <el-table-column prop="orderid" label="关联订单" width="220" />
        <el-table-column prop="count" label="入库数量" width="180" />
        <el-table-column prop="location" label="入库位置" width="180" />
...
import { getInList } from '@/api/refer/index';
...
const fetchData = async () => {
  try {
    ...
    const params = {
      ...
      size: pageSize.value,
      entryid: searchForm.value.entryid || undefined,
    ...
    const response = await getInList(params);
    ...
    if (response.code === 200) {
      ...
      console.error('获取入库列表失败:', response.msg);
      ElMessage.error(response.msg || '获取入库列表失败');
    }
  } catch (error) {
